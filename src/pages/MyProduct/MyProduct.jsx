    ...
    setProducts(prev => {
        return prev.map((item) => {
            if (item.id === editId) {
                return {
                    ...item,
                    product_image: item.product_image.filter((img) => img.id !== image.id)
                }
            }
            return item;
        })
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-10 mx-auto place-items-center">
                  {products.map((item, index) => (
                    ...
                    >
                      ...
                      <button onClick={() => openEditModal(item)} className="btn btn-warning w-full">Edit</button>
                      <button onClick={() => handleDelete(item)} className="btn btn-error w-full">Delete</button>
                    </div>
                  ))}
                </div>
              </>
